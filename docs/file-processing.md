...
Этот сценарий отличается от общего чата шаблона, описанного ниже: **файлы не загружаются в постоянное хранилище**. POST `/api/chat` принимает текст и список `{name, data}` с base64. Проверки размера, структуры Office/PDF и параметров изображений выполняются на сервере.
...
## Исходный шаблон: загрузки и RAG
