...
dev:
	...
	docker compose -f docker-compose.dev.yml exec -T app hackalem_ai_project_tryhackme db upgrade
	@echo ""
...
seed:
	...
	@if docker compose -f docker-compose.dev.yml exec -T app \
		hackalem_ai_project_tryhackme user list 2>/dev/null \
		...
		docker compose -f docker-compose.dev.yml exec -T app \
			hackalem_ai_project_tryhackme user create \
...
stage:
	...
	docker compose -f docker-compose.yml exec -T app hackalem_ai_project_tryhackme db upgrade
...
prod:
	...
	@sleep 5
	docker compose --env-file backend/.env -f docker-compose.prod.yml exec -T app hackalem_ai_project_tryhackme db upgrade
...
db-init: docker-db
	...
	@sleep 8
	cd backend && uv run hackalem_ai_project_tryhackme db migrate -m "initial" || true
	cd backend && uv run hackalem_ai_project_tryhackme db upgrade
	@echo ""
	@echo "✅ Database initialized!"

db-migrate:
	@read -p "Migration message: " msg; \
	uv run --directory backend hackalem_ai_project_tryhackme db migrate -m "$$msg"

db-upgrade:
	uv run --directory backend hackalem_ai_project_tryhackme db upgrade

db-downgrade:
	uv run --directory backend hackalem_ai_project_tryhackme db downgrade

db-current:
	uv run --directory backend hackalem_ai_project_tryhackme db current

db-history:
	uv run --directory backend hackalem_ai_project_tryhackme db history

# === Server ===
run:
	uv run --directory backend hackalem_ai_project_tryhackme server run --reload

run-prod:
	uv run --directory backend hackalem_ai_project_tryhackme server run --host 0.0.0.0 --port 8000

routes:
	uv run --directory backend hackalem_ai_project_tryhackme server routes

# === Users ===
create-admin:
	@echo "Creating admin user..."
	uv run --directory backend hackalem_ai_project_tryhackme user create-admin

user-create:
	uv run --directory backend hackalem_ai_project_tryhackme user create

user-list:
	uv run --directory backend hackalem_ai_project_tryhackme user list
...
docker-db:
	...
	@echo "   Connection: postgresql://postgres:postgres@localhost:5432/hackalem_ai_project_tryhackme"
...
help:
	@echo ""
	@echo "hackalem_ai_project_tryhackme - Available Commands"
	@echo "======================================"
	@echo ""
	...
	@echo ""
	@echo "RAG:"
	@echo "  uv run hackalem_ai_project_tryhackme rag-ingest <path> -c <collection>  Ingest files"
	@echo "  uv run hackalem_ai_project_tryhackme rag-search <query> -c <collection>  Search"
	@echo "  uv run hackalem_ai_project_tryhackme rag-collections                     List collections"
	@echo "  uv run hackalem_ai_project_tryhackme rag-sources                         List sync sources"
	@echo "  uv run hackalem_ai_project_tryhackme rag-source-add                      Add sync source"
	@echo "  uv run hackalem_ai_project_tryhackme rag-source-sync <id>                Trigger sync"
	@echo ""
