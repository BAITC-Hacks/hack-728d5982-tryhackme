...
```bash
uv run hackalem_ai_project_tryhackme cmd rag-ingest ./docs/ --collection docs --recursive   # ingest files/folder
uv run hackalem_ai_project_tryhackme cmd rag-search "your question" --collection docs        # semantic search
uv run hackalem_ai_project_tryhackme cmd rag-collections                                     # list collections
uv run hackalem_ai_project_tryhackme cmd rag-stats                                           # chunk/vector counts
uv run hackalem_ai_project_tryhackme cmd rag-drop <collection> --yes                         # delete a collection
```
...
```bash
uv run hackalem_ai_project_tryhackme cmd rag-sources                  # list
uv run hackalem_ai_project_tryhackme cmd rag-source-add               # add (interactive)
uv run hackalem_ai_project_tryhackme cmd rag-source-sync --all        # trigger a sync
```
